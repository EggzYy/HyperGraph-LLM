...
def visualise_hnx_with_pyvis(
    ...
    *,
    ...
    # ── configure PyVis ─────────────────────────────────────────────────────
    net = Network(
        height=height, width=width, bgcolor="#ffffff", directed=False, notebook=notebook
    )
    if physics:
        net.barnes_hut()
    else:
    ...
    for nid, attrs in inc_g.nodes(data=True):
        if is_edge.get(nid):  # hyper-edge
            ...
            if level == "contextual_link":
                ...
                label = (
                    (edge_info.get("text", "")[:30] + "…")
                    if edge_info.get("text")
                    else ""
                )
        ...
        for hn in [n for n, flag in is_edge.items() if flag]:
            ...
            for i in range(len(neigh)):
                for j in range(i + 1, len(neigh)):
                    net.add_edge(neigh[i], neigh[j], title=f"via {hn}", physics=physics)
    else:
        for u, v, e_attrs in inc_g.edges(data=True):
            cert = e_attrs.get("certainty", 1.0)
            colour = _certainty_to_hex(cert)
            net.add_edge(
                u, v, color=colour, title=json.dumps(e_attrs, indent=2), physics=physics
            )
